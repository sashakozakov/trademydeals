						...
						<div class="portlet-body">
							<div class="table-toolbar">
								...
								<div class=" pull-right">
									<div class="btn-group">
									<a href="new_<?php echo $_GET['action'];?>.php">
											<button id="sample_editable_1_new" class="btn green" style="margin-bottom:20px;">
									  Add <?php echo ucfirst($_GET['action']);?>									</button></a>								</div>
								</div>
							</div><br>
							...
							<tbody>
								...
								$posts=getPostedAllFeature();
									...
									$posts=getPostedByType($_GET['action']);
								}
								//print_R($user);
								$count=0;
                                 foreach($posts as $post) {
								 $total_position = "";
								 $position = $post['promote_'.$action];
								 $position1 = $post['promote_'.$action.'1'];
								 $position2 = $post['promote_'.$action.'2'];
								 $position3 = $post['promote_'.$action.'3'];
								 $position4 = $post['promote_'.$action.'4'];
									if($position!=""){
										$total_position .=$position.","; 
									}
									if($position1!=""){
										$total_position .=$position1.","; 
									}
									if($position2!=""){
										$total_position .=$position2.","; 
									}
									if($position3!=""){
										$total_position .=$position3.","; 
									}
									
									if($position4!=""){
										$total_position .=$position4.","; 
									}
							...
							<!--<td>
								...
								<td>
											...
											<a href="edit_<?php echo $_GET['action'];?>.php?id=<?php echo $post['id'];?>"><button  class="btn orange btn-sm">
									Edit									</button></a>&nbsp;|&nbsp;<a onclick="return Confirm_Box();" href="delete_post.php?id=<?php echo $post['id'];?>&action=<?php echo $_GET['action'];?>"><button  class="btn red btn-sm">
									Delete									</button></a>
                                    <a href="/get<?php echo $_GET['action'];?>.php?id=<?php echo $post['id'];?>"><button  class="btn red btn-sm">View</button></a>
                                </td>
							</tr><?php }
							 $count=0; ?>
