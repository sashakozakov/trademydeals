<?php
session_start();
if(!isset($_SESSION['user_name']) and !isset($_SESSION['admin_id']) ) {
    header("Location: http://trademydeals.com");
    exit();
}
include('header.php');
?>
            ...
            <div class="col-lg-9">
              ...
              <?php } ?>  </div>
                <div class="col-lg-12">
                    ...
                    <br>
                        <div class="col-lg-2">
                        <p><a href="membershipuser.php"><button type="button" class="btn btn-lg btn-success btn2" style="margin-left:0px; margin-top:15px;" id="myads">My Ads</button></a></p>
                  ...
                  <div class="table-responsive">
                        <table class="table table-bordered">
							...
							$limit = "$start, $nol"; 
							$count = 1; 
                                $adssql=mysql_query("SELECT * FROM `ads` WHERE user_id='$user_id' LIMIT $limit");
                             $sn=0;
                            ...
                            <tr>
                                ...
                                <td style="padding-left:20px;" width="220px">
                                    <a class="btn btn-sm btn-success btn1" href="getads.php?id=<?php echo $ads['id']; ?>">View</a>
                                    <a class="btn btn-sm btn-success btn1" href="editads.php?id=<?php echo $ads['id']; ?>">Edit</a>
<!--                                    <button type="button" class="btn btn-sm btn-success btn1">Edit</button>-->
                                    <a class="btn btn-sm btn-success btn1" href="delAds.php?id=<?php echo $ads['id'];?>">Delete</a>
                                    <?php


                                    if(!$ads['status']): ?>
                                    <span>Please pay or promote free</span>
                                    <?php endif; ?>
<!--                                    <button type="button" class="btn btn-sm btn-success btn1">Extended</button>-->
                                </td>
                                        <?php                                      
                                        $databaseDate = $ads["exp_date"];
                                        $baseDate = strtotime($databaseDate);
                                        $todayDate = mktime(0, 0, 0, date("m"), date("d"), date("Y")); 
                                        $baseDate;
                                        $data1 = $todayDate + 172800;
                                        $data2 = $todayDate - 172800;
                                        if ($data2 < $baseDate && $baseDate > $data1){
                                        ?>
                                    <?php }?>
                        ...
                        to the query string.*/ 
                        $off_sql = mysql_query ("SELECT * FROM `ads` WHERE user_id='$user_id'") or die ("Error in query: $off_sql".mysql_error()); 
